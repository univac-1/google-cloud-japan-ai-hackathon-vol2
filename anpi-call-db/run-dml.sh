...
echo -e "${GREEN}=== DML実行開始（サンプルデータ投入） ===${NC}"
...
# DMLファイルを順番に実行（設定ファイルから読み込み）
for dml_file in "${DML_FILES[@]}"; do
    file_path="dml/${dml_file}"
    
    if [[ -f "$file_path" ]]; then
        echo -e "${YELLOW}実行中: ${dml_file}${NC}"
        ...
        if [[ $? -eq 0 ]]; then
            echo -e "${GREEN}✓ ${dml_file} の実行が完了しました${NC}"
        else
            echo -e "${RED}✗ ${dml_file} の実行でエラーが発生しました${NC}"
            exit 1
        fi
    else
...
echo -e "${GREEN}=== DML実行完了 ===${NC}"
echo -e "${GREEN}サンプルデータの投入が完了しました${NC}"

# データ投入確認
echo -e "${YELLOW}=== データ投入確認 ===${NC}"
echo "接続してデータを確認するには:"
